-- =====================================================================
-- Migration 0079: P1 fix — place_order() consolidates duplicate product lines.
-- The validation loop reads on-hand BEFORE any reservation is written, so two
-- lines for the same product each passed the check and then both reserved,
-- overselling stock. We aggregate p_lines by product up front (one check + one
-- reservation + one order_item per product). Everything else is identical to
-- the prior live version. Applied live to project bjevoybwufubtprkxbvb.
--
-- NOTE: the full function body is reproduced here so a fresh DB rebuilds it.
-- The single added block is the array_agg(...) consolidation before the
-- FOR UPDATE lock. See migrations 0005/0020/0042/0050 for its history.
-- =====================================================================
